...
const uint8perm = new Uint8Array(256).map(() => Math.floor(Math.random() * 256));
for (let i = 0; i < 256; i++) {
  p[i] = p[i + 256] = uint8perm[i];
}
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      for (let i = 0; i < octaves; i++) {
        noiseVal += noise2D(x * freq * noiseScale, y * freq * noiseScale) * amp;
        freq *= 2;
        amp *= persistence;
      }
      grid[y][x] = noiseVal;
...
function chaikin(pts: {x: number, y: number}[], iterations = 3) {
  if (pts.length < 3) return pts;
  let points = pts;
  for (let k = 0; k < iterations; k++) {
    const q: {x: number, y: number}[] = [points[0]];
    for (let i = 0; i < points.length - 1; i++) {
      q.push({ 
        x: points[i].x * 0.75 + points[i + 1].x * 0.25, 
        y: points[i].y * 0.75 + points[i + 1].y * 0.25 
      });
      q.push({ 
        x: points[i].x * 0.25 + points[i + 1].x * 0.75, 
        y: points[i].y * 0.25 + points[i + 1].y * 0.75 
      });
    }
    q.push(points[points.length - 1]);
    points = q;
  }
  return points;
}
...
const App = () => {
  ...
  useEffect(() => {
    ...
    // ===== 比例尺參數計算 =====
    let areaKm = 5; // 預設 5km
    if (scale.includes("1:5,000")) areaKm = 1;
    else if (scale.includes("1:10,000")) areaKm = 2;
    else if (scale.includes("1:25,000")) areaKm = 5;
    else if (scale.includes("1:50,000")) areaKm = 10;
    
    // pxPerKm = mapWidth / areaKm
    const pxPerKm = drawWidth / areaKm;

    // ===== 等高線間距 (間距加大提高稀疏感) =====
    const range = maxHeight - minHeight;
    let interval = 100; // Requested default
    if (range > 4000) interval = 200;
    else if (range > 1500) interval = 100;
    else if (range > 500) interval = 40;
    else interval = 20;

    const indexInterval = interval * 5;

    // ===== 畫等高線 (Marching Squares with Path Stitching & Chaikin Smoothing) =====
    const drawContourSet = (level: number, isIndex: boolean) => {
      ...
      for (let y = 0; y < GRID_SIZE - 1; y++) {
        for (let x = 0; x < GRID_SIZE - 1; x++) {
          ...
          const pTop = { x: x + interp(v1, v2, level), y: y };
          const pRight = { x: x + 1, y: y + interp(v2, v3, level) };
          const pBottom = { x: x + (1 - interp(v4, v3, level)), y: y + 1 };
          const pLeft = { x: x, y: y + (1 - interp(v1, v4, level)) };
          ...
          switch (c) {
            case 1: case 14: addSeg(pLeft, pBottom); break;
            case 2: case 13: addSeg(pBottom, pRight); break;
            case 3: case 12: addSeg(pLeft, pRight); break;
            case 4: case 11: addSeg(pTop, pRight); break;
            case 5: addSeg(pTop, pRight); addSeg(pLeft, pBottom); break;
            case 6: case 9: addSeg(pTop, pBottom); break;
            case 7: case 8: addSeg(pTop, pLeft); break;
            case 10: addSeg(pTop, pLeft); addSeg(pBottom, pRight); break;
          }
        }
      }
      ...
      const hash = (p: {x: number, y: number}) => `${p.x.toFixed(4)},${p.y.toFixed(4)}`;
      ...
      while (used.size < segments.length) {
        ...
        used.add(startIdx);
        currentPath.push(segments[startIdx][0], segments[startIdx][1]);

        let extended = true;
        while (extended) {
          extended = false;
          const tail = currentPath[currentPath.length - 1];
          const tailStr = hash(tail);
        ...
        // Try searching backward from start too
        extended = true;
        while (extended) {
          extended = false;
          const head = currentPath[0];
          const headStr = hash(head);
          for (let i = 0; i < segments.length; i++) {
            if (used.has(i)) continue;
            const s0 = segments[i][0];
            const s1 = segments[i][1];
            if (hash(s0) === headStr) {
              currentPath.unshift(s1);
              used.add(i);
              extended = true;
              break;
            } else if (hash(s1) === headStr) {
              currentPath.unshift(s0);
              used.add(i);
              extended = true;
              break;
            }
      ...
      ctx.beginPath();
      // Apply professional styles from user request
      ctx.strokeStyle = isIndex ? '#6b3010' : '#c8966a';
      ctx.lineWidth = isIndex ? 2.0 : 0.5;
      ctx.lineCap = 'round';
      ctx.lineJoin = 'round';

      paths.forEach(p => {
        // Apply Chaikin smoothing
        const smoothPath = chaikin(p, 3);
        ctx.moveTo(offsetX + smoothPath[0].x * cellWidth, offsetY + smoothPath[0].y * cellHeight);
        for (let i = 1; i < smoothPath.length; i++) {
          ctx.lineTo(offsetX + smoothPath[i].x * cellWidth, offsetY + smoothPath[i].y * cellHeight);
        }
      });
      ctx.stroke();

      // ===== 數字標籤 (精確位置) =====
      if (isIndex && paths.length > 0) {
        ...
        paths.forEach(path => {
          if (path.length > 15) { 
            ...
            const px = offsetX + p.x * cellWidth;
            const py = offsetY + p.y * cellHeight;
            
            // Draw small background for text clarity
    ...
    // 嚴格等差海拔繪製
    for (let el = Math.floor(minHeight / interval) * interval; el <= maxHeight; el += interval) {
      if (el < minHeight) continue;
      const isIndex = el % indexInterval === 0;
      drawContourSet(el, isIndex);
    }
    ...
    for (let y = 1; y < GRID_SIZE - 1; y++) {
      for (let x = 1; x < GRID_SIZE - 1; x++) {
        ...
        if (val > maxHeight * 0.8 && 
          ...
          ctx.fillText('▲', px - 7, py + 2);
          ctx.font = 'bold 10px sans-serif';
          ctx.fillText(Math.round(val).toString(), px + 4, py + 12);
        }
      }
    }
    ...
    // ===== 靜態指北標 (固定指北) =====
    ctx.save();
    ctx.translate(canvas.width - 60, MARGIN + 40);
    ctx.fillStyle = '#1c1917';
    ctx.beginPath();
    ctx.moveTo(0, -25);
    ctx.lineTo(8, 10);
    ...
    // ===== 比例尺條 ( pxPerKm = mapWidth / areaKm ) =====
    ctx.save();
    ctx.translate(offsetX + 10, canvas.height - MARGIN + 25);
    ctx.lineWidth = 1.5;
    ctx.strokeStyle = '#000';
    
    // pxPerKm is calculated above
    const scaleBarWidth = pxPerKm; // Represent 1km
    
    ctx.beginPath();
    ctx.moveTo(0, 0); ctx.lineTo(scaleBarWidth, 0);
    ctx.moveTo(0, -4); ctx.lineTo(0, 4);
    ctx.moveTo(scaleBarWidth/2, -4); ctx.lineTo(scaleBarWidth/2, 4);
    ctx.moveTo(scaleBarWidth, -4); ctx.lineTo(scaleBarWidth, 4);
    ctx.stroke();
    ...
    ctx.textAlign = 'center';
    ctx.fillText('0', 0, 15);
    ctx.fillText('500m', scaleBarWidth/2, 15);
    ctx.fillText('1km', scaleBarWidth, 15);
    
    ctx.font = 'bold 12px sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText(`比例尺 ${scale}`, scaleBarWidth + 15, 0);
    ctx.restore();
    ...
    if (profileLine) {
      ...
      ctx.beginPath(); ctx.arc(profileLine.start.x, profileLine.start.y, 4, 0, Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc(profileLine.end.x, profileLine.end.y, 4, 0, Math.PI*2); ctx.fill();
      
      ctx.font = 'bold 14px sans-serif';
      ctx.fillText('A', profileLine.start.x - 5, profileLine.start.y - 10);
      ctx.fillText('B', profileLine.end.x + 5, profileLine.end.y - 10);
    }

  }, [terrain, minHeight, maxHeight, scale, profileLine]);

  // --- Profile Chart Component ---
  const ProfileChart = () => {
    if (!profileLine || !terrain) return null;
  ...
  const generateQuestions = async () => {
    ...
    try {
      ...
      const parts: any[] = [
        {
          text: `你是一位地理老師。請根據以下地形參數與特徵，生成 3 題適合高中程度的地理素養試題。
        ...
        <div className="card space-y-8">
          ...
          <div className="grid grid-cols-1 gap-4 pt-4 border-t border-stone-100">
            <button 
              ...
              className="flex items-center justify-center gap-2 py-4 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100"
            >
              <MapIcon size={20} />
              生成等高線地形圖
            </button>
          ...
          <div className="card overflow-hidden p-0">
            <div className="p-4 border-b border-stone-100 flex items-center justify-between bg-white">
              ...
              <div className="flex gap-2">
                ...
                >
                  {isLoading && terrain.length === 0 ? <RefreshCw size={18} className="animate-spin" /> : <FileText size={18} />}
                  立即產出 AI 模擬試題
                </button>
              </div>
            </div>
